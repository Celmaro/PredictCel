...
def test_active_positions_count_as_held_and_exposed(tmp_path) -> None:
    store = SignalStore(str(tmp_path / "predictcel.db"))
...
def test_update_position_can_close_active_position(tmp_path) -> None:
    store = SignalStore(str(tmp_path / "predictcel.db"))
